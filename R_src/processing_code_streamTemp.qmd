---
title: "processing data"
format: html
editor: source
---

## Set up

```{r libraries, message=FALSE, warning=FALSE}

library(tidyverse)
library(readr)
library(archive)
library(sysfonts)
library(png)
library(webp)
```

## 1. Fetch

Activate ScienceBase credentials. Note, you only have to do this once to get the data the first time.

1.  Run the code below
2.  When prompted, enter your email address as the username
3.  Log into https://sciencebase.usgs.gov/manager/ and select "Copy API Token" from the User dropdown menu
4.  Copy the Token into the pop up dialog. If this worked, you should get a "TRUE" result in the console.

```{r SB, eval=FALSE}
# if file exists, skip
if(!file.exists("in/sciencebase/base10.csv")){
  
  sbtools::initialize_sciencebase_session()
  
  ### Fetch data from ScienceBase.
  
  # Benjamin, J.R., Dunham, J.B., and Penn, C.A., 2023, 
  # Simulated growth potential of redband trout in the Donner und Blitzen River 
  # Basin, southeastern Oregon, using a bioenergetics model: U.S. Geological 
  # Survey data release, https://doi.org/10.5066/P9MAXBZN.
  
  
  sbtools::item_file_download(sb_id = "65381be6d34ee4b6e05bba62",
                              dest_dir = "in/sciencebase",
                              overwrite_file = FALSE)
  
  # Penn, C.A., Dunham, J.B., Markstrom, S.L., Overstreet, B.T., and Stratton, 
  # L.E., 2023, Simulated streamflow and stream temperature in the Donner und 
  # Blitzen River Basin, southeastern Oregon, using the Precipitation-Runoff 
  # Modeling System (PRMS): U.S. Geological Survey data release,
  # https://doi.org/10.5066/P9WQM25Y.
  
  sbtools::item_file_download(sb_id = "644a93add34ee8d4adee04be",
                              names = "Blitzen_PRMS_output.zip",
                              destinations = "in/second_data_release/Blitzen_PRMS_output.zip",
                              overwrite_file = FALSE)
  
  archive::archive_extract(
    archive = "in/second_data_release/Blitzen_PRMS_output.zip",
    dir = "in/second_data_release")
}
```


Read in data for stream temperature

```{r fetch_temperature}
segment_temp <- readr::read_csv("in/second_data_release/nsegment_summary_seg_tave_water.csv", 
                        show_col_types = FALSE)



```


## 2. Process for Figure 3



```{r processing}
ave_temps <- segment_temp |> 
  pivot_longer(cols = !c(Date), names_to = "Segment", values_to = "Temperature") |>
  mutate(month = lubridate::month(Date)) |>
  group_by(Segment, month) |>
  summarize(mean_temp = mean(Temperature, na.rm = TRUE))

representative_segments <- ave_temps |> filter(Segment %in% c(51)) #99, 65

segment51 <- segment_temp |> rename(segment51 = "51") |> select(segment51, Date) |>
  mutate(year = lubridate::year(Date),
         yday = lubridate::yday(Date),
         month = lubridate::month(Date)) |>
  group_by(year, month) |>
  summarize(monthly_mean_temp = mean(segment51, na.rm = TRUE))
  #filter(yday > 100, yday < 300) 

```

```{r page08}
#font_main <- "Just Another Hand"
font_main <- "Source Sans Pro"
sysfonts::font_add_google(font_main)

color_cold_optimal <- "#7EACB5"
color_cold_stress <- '#7A3608'#"#C96868"
color_warm_optimal <- "#7EACB5"
color_warm_stress <- '#7A3608'#"#C96868"

library(geomtextpath)
color_cold_optimal <- "#7EACB5"
color_cold_stress <- '#7A3608'#"#C96868"
color_warm_optimal <- "#7EACB5"
color_warm_stress <- '#7A3608'#"#C96868"


ggplot(data = representative_segments,  
       aes(y = mean_temp, x = month, color = Segment)) +
  # y-axis
  annotate(geom = "text", y = c(0, 10, 20, 30), x = rep(0.5, 4), 
           label = c(0, 10, 20, 30), 
           size = 16, color = "grey30", hjust = 1) +
  annotate(geom = "text", y = 30, x = 0.6, label = "degrees Celsius",
           size = 16, color = "grey30", hjust = 0) +
  annotate(geom = "line", y = rep(0,2), x = c(0.7,12), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = rep(10,2), x = c(0.7,12), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = rep(20,2), x = c(0.7,12), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = rep(30,2), x = c(3.7,12), linewidth = 0.8, color = "grey60") +
  # x-axis
  annotate(geom = "text", y = rep(-2, 12), x = 1:12, 
           label = c("J","F","M","A","M","J","J","A","S","O","N","D"), 
           size = 16, color = "grey30", hjust = 0.5) +
  annotate(geom = "line", y = c(-1,0.5), x = rep(1,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(2,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(3,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(4,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(5,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(6,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(7,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(8,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(9,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(10,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(11,2), linewidth = 0.8, color = "grey60") +
  annotate(geom = "line", y = c(-1,0.5), x = rep(12,2), linewidth = 0.8, color = "grey60") +
  # annual fluctuations
  geom_line(data = segment51,
            aes(y = monthly_mean_temp, x = month, group = year),
            linewidth = 0.5, color = color_warm_optimal) +  
  # thermal stress thresholds
  annotate("line", x = c(1, 12), 
           y = c(24.1, 24.1), alpha = 0.7, 
           color = color_cold_stress, linewidth = 4) +  
  annotate("text", label = "Thermal stress threshold\nfor warm-adapted fish", 
           color = color_cold_stress,
           x = 0.5, y = 26.2, fontface = "bold",
           hjust = 0, size = 16, lineheight = 0.8) +
  # Average across time 1979-2021
  geom_line(linewidth = 6, color = "#458869")  +
  # title
  annotate(geom = "text", y = 32, x = 0,          
           label = "Mean monthly stream temperature from 1979-2021",
           color = "black", hjust = 0, size = 18, vjust = 0, fontface = "bold") +
  # explainer label
  annotate(geom = "text", y = 8, x = 5, label = "1979-2021 average in bold",
           color = "#376D54", hjust = 0, size = 16, fontface = "bold") +
  annotate(geom = "text", y = 6, x = 6, label = "Annual values in light",
           color = "#376D54", hjust = 0, size = 16) +
  geom_textpath(data = segment51 |> filter(year == 2007), label = "Annual values",
            aes(y = monthly_mean_temp, x = month), size = 14,
            linewidth = 0, color = color_warm_optimal) +
  ylim(c(-2, 34)) +
  xlim(c(-0, 12)) +
  theme_void() +
  theme(panel.background = element_rect(fill = "white"),
        plot.background = element_rect(fill = "white", color = NA))

ggsave(filename = "../public/assets/vectorized_pngs/page12.png", 
       width = 3000, height = 2300, dpi = 150, units = "px")

# Convert to webp
img <- png::readPNG("../public/assets/vectorized_pngs/page12.png")
webp::write_webp(img, "../public/assets/vectorized_pngs/page12.webp")
```




